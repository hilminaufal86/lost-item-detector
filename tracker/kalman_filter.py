...
class KalmanFilter(object):
    ...
    def predict(self, mean, covariance):
        ...
        motion_cov = np.diag(np.square(np._r[std_pos, std_vel]))

        mean = np.dot(self._motion_matrix, mean)
        covariance = np.linalg.multi_dot((self._motion_matrix, covariance, self._motion_matrix.T)) + motion_cov
    ...
    def update(self, mean, covariance, measurement):
        ...
        cholesky_factor, lower = scipy.linalg.cho_factor(projected_cov, lower=True, check_finite=False)
        kalman_gain = scipy.linalg.cho_solve((cholesky_factor, lower), np.dot(covariance, self._update_matrix.T).T, check_finite=False).T
